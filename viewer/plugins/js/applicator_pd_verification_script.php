<script type="text/javascript">
	...
	document.addEventListener("DOMContentLoaded", () => {
		document.getElementById("vc_emp_no_search").focus();
	});
	...
	sessionStorage.setItem('zs_vc_emp_no_search', '');
	sessionStorage.setItem('zs_vc_applicator_no_search', '');

	document.getElementById("vc_emp_no_search").addEventListener("input", e => {
		delay(function () {
			if (document.getElementById("vc_emp_no_search").value.length < 256) {
				document.getElementById("vc_emp_no_search").value = "";
			}
			let vc_emp_no = sessionStorage.getItem('zs_vc_emp_no_search');
			if (vc_emp_no != '') {
				document.getElementById("vc_emp_no_search").value = vc_emp_no;
				sessionStorage.setItem('zs_vc_emp_no_search', '');
			}
		}, 150);
	});

	document.getElementById("vc_emp_no_search").addEventListener("change", e => {
		e.preventDefault();
		sessionStorage.setItem('zs_vc_emp_no_search', e.target.value);
		document.getElementById("vc_emp_no_search").disabled = true;
	...
	const reset_vc_applicator_no_search_field = opt => {
		...
		document.getElementById("vc_applicator_no_search").disabled = true;
		document.getElementById("vc_emp_no_search").value = '';
		document.getElementById("vc_emp_no_search").disabled = false;
		document.getElementById("vc_emp_no_search").focus();
	}
	...
	const verify_emp_no = () => {
		let emp_no = document.getElementById('vc_emp_no_search').value;
		...
		$.ajax({
			...
			dataType: 'json',
			data: {
				method: 'verify_emp_no',
				emp_no: emp_no
			},  
			success: response => {
				if (response.message == 'success') {
					document.getElementById('vc_full_name_hidden').value = response.full_name;
					get_ac_details();
				} else {
	...
	const verify_checksheet = () => {
		let emp_no = document.getElementById("vc_emp_no_search").value;
		let full_name = document.getElementById("vc_full_name_hidden").value;
